...
export default function Dashboard() {
  ...
  const status =
    open === 0 && goals.length === 0
      ? "Fresh core. Give me a task, a goal, or just talk to me — say “hey shaffa”."
      : `${open} open task${open === 1 ? "" : "s"} · ${goals.length} goal${goals.length === 1 ? "" : "s"} tracked · say the word and we begin.`;
  ...
  return (
    <PageTransition className="mx-auto flex max-w-6xl flex-col gap-5">
      ...
      >
        {[TodayOverview, UpcomingDeadlines, FocusTimer, ProductivityScore, RecentCommands, DailyInsights].map(
          (Widget, i) => (
